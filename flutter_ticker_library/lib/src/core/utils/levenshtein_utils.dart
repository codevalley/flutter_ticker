...
class LevenshteinUtils {
  ...
  /// @return an int array where each index corresponds to one of [ACTION_SAME], [ACTION_INSERT],
  ///         [ACTION_DELETE] to represent if we update, insert, or delete a character
  ///         at the particular index.
  static List<int> computeColumnActions(
      List<String> source, 
      List<String> target,
    ...
    while (true) {
      ...
      if (reachedEndOfSource && reachedEndOfTarget) {
        ...
        _fillWithActions(columnActions, target.length - targetIndex, ACTION_INSERT);
        break;
      } else if (reachedEndOfTarget) {
        _fillWithActions(columnActions, source.length - sourceIndex, ACTION_DELETE);
        break;
      }

      final bool containsSourceChar = supportedCharacters.contains(source[sourceIndex]);
      final bool containsTargetChar = supportedCharacters.contains(target[targetIndex]);

      if (containsSourceChar && containsTargetChar) {
        // We reached a segment that we can perform animations on
        final int sourceEndIndex =
            _findNextUnsupportedChar(source, sourceIndex + 1, supportedCharacters);
        final int targetEndIndex =
            _findNextUnsupportedChar(target, targetIndex + 1, supportedCharacters);

        _appendColumnActionsForSegment(
          columnActions,
          source,
          target,
          sourceIndex,
  ...
  static void _appendColumnActionsForSegment(
    ...
    final int resultLength = sourceLength > targetLength ? sourceLength : targetLength;

    if (sourceLength == targetLength) {
      // No modifications needed if the length of the strings are the same
      _fillWithActions(columnActions, resultLength, ACTION_SAME);
      return;
    }
  ...
  static int _min(int first, int second, int third) {
    return [first, second, third].reduce((curr, next) => curr < next ? curr : next);
  }
}
